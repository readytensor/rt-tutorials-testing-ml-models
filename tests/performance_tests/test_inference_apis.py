...
def test_api_endpoint_performance(
    ...
    store_results_to_csv(
        ...
        ("endpoint", "num_requests_tested", "avg_response_time_secs", "throughput_reqs_per_sec"),
        (endpoint.lstrip('/'), num_requests, round(response_time, 4), round(throughput, 4))
    )


def run_inference_api_and_record_metrics(app, endpoint, sample_request_data, num_requests=50):
    ...
    Returns:
        tuple: The average response time in seconds and the throughput in requests per second.
    """
    # For computing both response time and throughput, send multiple requests and measure the time
