...
def test_type_caster_string_casting():
    ...
    result = type_caster.fit_transform(data)
    expected_result = pd.DataFrame({'A': ['1', '2'], 'B': ['3.0', '4.0'], 'C': [5.0, 6.0]})
    pd.testing.assert_frame_equal(result, expected_result)
...
def test_type_caster_mix_null_non_null_values():
    """
    Test TypeCaster transformer with columns containing a mix of null and non-null values.
...
def test_most_frequent_imputer_valid_cat_vars_threshold():
    ...
    It should impute the missing values with the most frequent class in the specified columns.
    """
    df = pd.DataFrame({"col1": ['a', 'a', np.nan, 'b'], "col2": ['x', np.nan, np.nan, np.nan]})
    mfi = MostFrequentImputer(cat_vars=["col1", "col2"], threshold=0.5)
    result = mfi.fit_transform(df)
    expected = pd.DataFrame({"col1": ['a', 'a', 'a', 'b'], "col2": ['x', np.nan, np.nan, np.nan]})
    pd.testing.assert_frame_equal(result, expected)
...
def test_most_frequent_imputer_invalid_cat_vars():
    ...
    It should not change the input DataFrame.
    """
    df = pd.DataFrame({"col1": ['a', 'a', np.nan, 'b'], "col2": ['x', 'y', 'x', np.nan]})
    mfi = MostFrequentImputer(cat_vars=["invalid"], threshold=0.5)
    result = mfi.fit_transform(df)
    expected = pd.DataFrame({"col1": ['a', 'a', np.nan, 'b'], "col2": ['x', 'y', 'x', np.nan]})
    pd.testing.assert_frame_equal(result, expected)
...
def test_most_frequent_imputer_mix_valid_invalid_cat_vars():
    ...
    result = mfi.fit_transform(df)
    expected = pd.DataFrame({"col1": ['a', 'a', 'a', 'b'], "col2": ['x', 'y', 'x', np.nan]})
    pd.testing.assert_frame_equal(result, expected)
...
def test_most_frequent_imputer_no_missing_values():
    ...
    mfi = MostFrequentImputer(cat_vars=["col1", "col2"], threshold=0.5)
    result = mfi.fit_transform(df)
    expected = pd.DataFrame({"col1": ['a', 'a', 'b', 'b'], "col2": ['x', 'y', 'x', 'y']})
    pd.testing.assert_frame_equal(result, expected)
...
def test_most_frequent_imputer_all_missing_values():
    ...
    mfi = MostFrequentImputer(cat_vars=["col1", "col2"], threshold=0.5)
    result = mfi.fit_transform(df)
    expected = pd.DataFrame({"col1": [np.nan, np.nan, np.nan, np.nan], "col2": [np.nan, np.nan, np.nan, np.nan]})
    pd.testing.assert_frame_equal(result, expected)
...
def test_feature_engine_transformer_wrapper_valid_transformer_valid_vars_valid_kwargs():
    """
    Test TransformerWrapper with a valid transformer, valid categorical variables, and valid kwargs.
    """
    df = pd.DataFrame({"col1": [np.nan, "A", "B", np.nan], 
                       "col2": [1, 2, 3, 4], "col3": ["X", np.nan, "Y", np.nan]})
    ...
    expected = pd.DataFrame({"col1": ["Unknown", "A", "B", "Unknown"], 
                             "col2": [1, 2, 3, 4], "col3": ["X", "Unknown", "Y", "Unknown"]})
    cols = ["col1", "col2", "col3"]
...
def test_feature_engine_transformer_wrapper_invalid_transformer():
    ...
    """
    df = pd.DataFrame({"col1": [np.nan, "A", "B", np.nan], "col2": [1, 2, 3, 4], "col3": ["X", np.nan, "Y", np.nan]})
...
def test_feature_engine_transformer_wrapper_invalid_vars():
    ...
    """
    df = pd.DataFrame({"col1": [np.nan, 1, 2, np.nan], 
                       "col2": [1, 2, 3, 4], 
...
def test_valid_ohe_columns_and_max_num_categories():
    ...
    X = pd.DataFrame({
        ...
        'shape': ['circle', 'square', 'circle', 'square', 'square', 'circle', 'square']
    })
    ...
    for max_num_categories, expected_num_cols in zip(max_num_categories_values, expected_num_cols_arr):
...
def test_ohe_columns_with_more_than_max_num_categories():
    ...
    'color': ['red', 'blue', 'green', 'red', 'red', 'green', 'blue'],
    'size': ['small', 'medium', 'large', 'medium', 'small', 'small', 'medium'],
    'material': ['wood', 'steel', 'plastic', 'steel', 'wood', 'wood', 'plastic'],
    'shape': ['circle', 'square', 'triangle', 'square', 'square', 'circle', 'square']
    })
    ohe_columns = ['color', 'size', 'material', 'shape']
