...
def test_init(default_hyperparameters, hpt_specs, hpt_results_file_path):
    ...
    """
    tuner = SKOHyperparameterTuner(default_hyperparameters, hpt_specs, hpt_results_file_path)
    ...
    assert tuner.is_minimize == True
    assert tuner.num_trials == hpt_specs.get("num_trials", 20)
    assert tuner.hyperparameter_names == ["hp_int", "hp_float", "hp_log_int", "hp_log_float", "hp_categorical"]
...
def test_get_objective_func(mocker, tuner, mock_data, default_hyperparameters):
    ...
    mock_train.assert_called_once_with(mock_train_X, mock_train_y, default_hyperparameters)
...
def test_run_hyperparameter_tuning(mocker, tuner, mock_data):
    ...
    result = tuner.run_hyperparameter_tuning(mock_train_X, mock_train_y, mock_valid_X, mock_valid_y)
...
def test_get_best_hyperparameters(mocker, tuner):
    ...
    mock_optimizer_results.x_iters = [[1, 1.0, 1, 0.1, "a"], [2, 2.0, 2, 0.2, "b"], [3, 3.0, 3, 0.3, "c"]]
...
def test_save_hpt_summary_results(mocker, tuner, tmpdir):
    ...
    assert df.shape == (3, 7)
    assert (df.columns == [
        "trial_num", "hp_int", "hp_float", "hp_log_int", 
        "hp_log_float", "hp_categorical", "metric_value"]).all()



def test_tune_hyperparameters(
        mocker, mock_data, hpt_results_file_path, 
        default_hyperparameters_file_path, default_hyperparameters,
    ...
    result = tune_hyperparameters(
        ...
        hpt_results_file_path, is_minimize,
        default_hyperparameters_file_path, hpt_specs_file_path)
